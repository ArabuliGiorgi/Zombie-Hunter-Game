...
function Pistol.new(x, y, fireRate, damage)
    ...
    self.image = love.graphics.newImage('Sprites/Weapon/M1911/m1911.png')
    self.bulletImage = love.graphics.newImage('Sprites/Weapon/M1911/.45 acp.png')
    self.state = "idle" -- "idle" or "shoot"

    -- Audio
    self.shootSound = love.audio.newSource("Audio/pistol.mp3", "static")
...
function Pistol:shoot(targetX, targetY)
    if self.cooldown <= 0 then
        self.state = "shoot"
        ...
        local bullet = Bullet:new(
            self.x + self.offsets.bulletX,
            self.y + self.offsets.bulletY,
            targetX,
            targetY,
            self.damage,
            500,
...
function Pistol:draw()
    ...
    if self.muzzle.active then
        ...
        love.graphics.draw(
            self.muzzle.image,
            quad,
            self.x + self.offsets.muzzleX, 
            self.y + self.offsets.muzzleY, 
            0,
            0.7,
            0.7
        )
...
function Pistol:BulletsCollision(zombie)
    for _, b in ipairs(self.bullets) do
